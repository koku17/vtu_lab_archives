...
for i in df.columns:
	...
	plt.savefig(f'histogram_{i}.svg')
	plt.xlabel(i)
	plt.ylabel('Frequency')
	plt.boxplot(df[i], vert = False)
	plt.title(f'Boxplot of {i}')
	plt.savefig(f'box_plot_{i}.svg')
	plt.show()
...
plt.savefig('corr_matrix_heatmap.svg')
plt.show()
...
for i in numerical:
	...
	df[i].hist(bins=20, alpha=0.5, color='b',edgecolor='black')
	plt.title(f'Histogram of {i}')
	plt.savefig(f'histogram2_{i}.svg')
	plt.xlabel(i)
	plt.ylabel('Frequency')
	plt.boxplot(df[i], vert=False)
	plt.title(f'Boxplot of {i}')
	plt.savefig(f'boxplot2_{i}.svg')
	plt.show()
...
for col in categorical:
	...
	plt.savefig(f'countplot_{col}.svg')
	plt.show()
...
plt.yticks(rotation=0)
plt.title("Correlation Matrix Heatmap")
plt.savefig("corr_matrix_heatmap2.svg")
plt.show()
...
plt.savefig('polynomial_regression.svg')
plt.show()
